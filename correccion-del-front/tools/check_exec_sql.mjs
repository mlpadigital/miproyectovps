...
async function checkExecSql() {
  const testCalls = ['exec_sql', 'execute_sql', 'sql', 'run_sql'];
  for (const name of testCalls) {
    const { data, error } = await supabaseAdmin.rpc(name, { query: 'SELECT 1;' });
    console.log(`RPC "${name}":`, error ? error.message : data);
  }
}
