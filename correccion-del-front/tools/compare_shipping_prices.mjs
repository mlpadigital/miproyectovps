...
async function compareRates() {
  ...
  console.log('=== TEST 1 (2 SEPARATE PACKAGES SENT) ===');
  for (const r of (res1.data?.data || [])) {
    console.log(`${r.carrierDescription} - ${r.serviceDescription}: $${r.totalPrice}`);
  }

  console.log('\n=== TEST 2 (1 CONSOLIDATED PACKAGE SENT) ===');
  for (const r of (res2.data?.data || [])) {
    console.log(`${r.carrierDescription} - ${r.serviceDescription}: $${r.totalPrice}`);
  }
}
