...
function extractHelmetData(content, filePath, routes) {
  ...
  const url = routes && routes.has(fileName)
    ? routes.get(fileName)
...
function main() {
  ...
  if (pages.length === 0) {
    // Generate a default entry if no pages found
    pages = [{
      url: '/',
      title: 'MlpaDigital',
      description: 'Escala tu Negocio Globalmente'
    }];
  }
...
const isMainModule = true; // Simplified for this environment
