...
async function testDelete() {
  ...
  // Now try delete as anon without session
  const { data: delAnon, error: delErr } = await anonClient
    .from('store_customers')
    .delete()
    .eq('id', inserted.id)
    .select();

  console.log('Anon delete result (without login):', delAnon, delErr);

  // Let's check RLS on store_customers
  const { data: checkAfterAnon } = await supabaseAdmin.from('store_customers').select('*').eq('id', inserted.id);
  console.log('Exists after anon delete?', checkAfterAnon.length > 0);
